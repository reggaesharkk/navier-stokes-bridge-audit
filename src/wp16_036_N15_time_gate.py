"""Prospective frozen N15 K36 static and time-resolved gate evaluator."""
...
def run(n14_path,n15_path,source_path,out_path):
    j14=json.loads(n14_path.read_text())
    j15=json.loads(n15_path.read_text())
    source=json.loads(source_path.read_text())
    if j15.get("cutoffs")!=[15] or j15.get("resume_from_N")!=14:
        raise ValueError("N15 continuation structure changed")
    row15=hold.get_row(j15,15)
    if row15.get("seed")!=20260940 or row15.get("search",{}).get("search_grid")!=48:
        raise ValueError("N15 continuation does not match frozen schedule")
    hold.System=DealiasedSystem
    system,states=hold.reconstruct(hold.get_row(j14,14),row15)
    keys=hold.frozen_keys(source)
    result={
        "status":"prospective frozen N15 time-resolved K36 evaluation",
        "input_sha256":{"n14":sha(n14_path),"n15":sha(n15_path),"source":sha(source_path)},
    ...
    for name,a0 in states.items():
        ...
        print("N15",name,"static",result["states"][name]["static_gate_pass"],
...
if __name__=="__main__":
    ...
    for name in ("n14-json","n15-json","source-json","output"):
        p.add_argument("--"+name,type=Path,required=True)
    a=p.parse_args()
    run(a.n14_json,a.n15_json,a.source_json,a.output)
