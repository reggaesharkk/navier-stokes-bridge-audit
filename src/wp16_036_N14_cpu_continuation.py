"""Frozen N14 phase continuation with bounded memory and atomic checkpoints.
...
N13_SHA256 = "13e5e56676b9398e7c7ec32f55e32a4d07dec5a3830c67787c6cd6fb5c8e59cd"
SEED = 20260939
GRID = 48
GLOBAL_DRAWS = 16
NEW_ROUNDS = 3
...
def run(n13_path, checkpoint_path, output_path, stop_after_trial, chunk):
    if sha(n13_path)!=N13_SHA256:
        raise ValueError("N13 input hash differs from the frozen continuation")
    source=json.loads(n13_path.read_text(encoding="utf-8"))
    if source.get("cutoffs")!=[13] or source.get("resume_from_N")!=12:
        raise ValueError("N13 continuation structure differs")
    prev=source["rows"][0]
    if prev["N"]!=13:
        raise ValueError("N13 row missing")
    fingerprints={
        "n13_json": N13_SHA256,
    ...
    started=time.time()
    print("BUILD N14 MODE AND SOURCE TABLE",flush=True)
    system=LowMemorySearchSystem(14,nu=NU)
    print("N14 high ordered sources",len(system.high_out),flush=True)
    ...
    if checkpoint_path.exists():
        ...
        if cp["fingerprints"]!=fingerprints or cp["N"]!=14 or cp["seed"]!=SEED:
    ...
    def save(trial):
        atomic_json(checkpoint_path,{
            "N":14,"seed":SEED,"search_grid":GRID,
    ...
    for trial in range(last+1,TOTAL+1):
        ...
        if q["C_infinity_stretch"]>best["C_infinity_stretch"]:
            ...
            records.append(row)
        save(trial)
        print(f"N14 trial {trial}/{TOTAL} C={best['C_infinity_stretch']:.9f}",flush=True)
    ...
    for grid in (48,64,96,128):
        ...
        print("N14 REFINE",grid,refined[str(grid)]["C_infinity_stretch"],flush=True)
    row={
        "N":14,"anchor_time":float(prev["anchor_time"]),
    ...
    result={"status":"completed frozen N14 cutoff continuation",
            "resume_source":str(n13_path),"resume_from_N":13,"cutoffs":[14],
            "anchor_time":row["anchor_time"],"rows":[row],
            "best_refined_case":{"N":14,"C":refined["128"]["C_infinity_stretch"]},
            "interpretation_rule":"finite phase search; not a global optimum or PDE theorem"}
    atomic_json(output_path,result)
    save(TOTAL)
    print("N14 COMPLETE",output_path,flush=True)
...
if __name__=="__main__":
    ...
    run(a.n13_json,a.checkpoint,a.output,a.stop_after_trial,a.chunk)
